...
/*
export function matchURLs1(str: string) {
  const result: Position[] = [];
  const begin = 0;
  let end = str.length;

  const isProtocolSymbol = (c: number) => {
    if (c < 0x80) {
      return isAlphaOrDigit(String.fromCharCode(c)) || c == "+".charCodeAt(0) ||
        c === "-".charCodeAt(0);
    }
    return getUnicodeSimpleCategory(c) != UnicodeSimpleCategory.Separator;
  };

  const isUserDataSymbol = (c: number) => {
  ...
  const isDomainSymbol = (c: number) => {
    if (c < 0xc0) {
      const char = String.fromCharCode(c);
      return char === "." || isAlphaDigitUnderscoreOrMinus(char) ||
        char === "~";
    }
    return isURLUnicodeSymbol(c);
  };
  ...
  while (true) {
    ...
    while (domainBeginPos != begin) {
      domainBeginPos = domainBeginPos - 1;
      const nextPos = domainBeginPos;
      const code = str.charCodeAt(nextPos - 1);
    ...
    let lastAtPos: number | undefined;
    let domainEndPos = begin + dotPos;
    while (domainEndPos != end) {
      const nextPos = domainEndPos + 1;
      if (str[nextPos] === "@") lastAtPos = domainEndPos;
      else if (!isDomainSymbol(str.charCodeAt(nextPos))) break;
    ...
    if (lastAtPos) {
      while (domainBeginPos != begin) {
        ...
        if (!isUserDataSymbol(str.charCodeAt(nextPos))) {
          domainBeginPos = nextPos;
          break;
        }
      }
    ...
    console.log({ dotPos, domainBeginPos, domainEndPos });
    console.log("Domain:", str.substring(domainBeginPos, domainEndPos));
    ...
    console.log("Domain_port:", str.substring(domainBeginPos, urlEndPos));
    ...
    if (
      ...
      while (pathEndPos != end) {
        ...
        if (!isURLPathSymbol(str.charCodeAt(nextPos))) break;
        pathEndPos = nextPos;
      }
      while (
    ...
    console.log("Domain_port_path:", str.substring(domainBeginPos, urlEndPos));

    let isBad = false;
    let urlBeginPos = domainBeginPos;
    if (urlBeginPos != begin && str[urlBeginPos - 1] === "@") {
      if (lastAtPos) isBad = true;
      ...
      if (userDataBeginPos == urlBeginPos - 1) isBad = true;
      urlBeginPos = userDataBeginPos;
    }

    console.log("User_data_port_path:", str.substring(urlBeginPos, urlEndPos));
    ...
    if (urlBeginPos != begin) {
      ...
      if (prefix.length >= 6 && prefix.endsWith("://")) {
        ...
        while (protocolBeginPos != begin) {
          ...
          if (!isProtocolSymbol(str.charCodeAt(nextPos))) {
        ...
        const code = str.charCodeAt(prefixEnd - 1);
        const char = str[prefixEnd - 1];
        if (
    ...
    console.log("Full:", str.substring(urlBeginPos, urlEndPos), isBad);

    if (!isBad) {
      if (urlEndPos > begin + dotPos + 1) {
        result.push([urlBeginPos, urlEndPos]);
      }
...
export function matchURLs(str: string) {
  ...
  while (true) {
    ...
    while (domainEndPos != end) {
      const nextPos = domainEndPos + 1;
      const code = str.charCodeAt(nextPos);
      if (str[nextPos] === "@") {
    ...
    if (urlBeginPos != begin) {
      const prefix = str.substring(begin, urlBeginPos); // CHECK: +1? really?
      if (prefix.length >= 6 && prefix.endsWith("://")) {
        let protocolBeginPos = urlBeginPos - 3;
        while (protocolBeginPos != begin) {
          protocolBeginPos--;
        ...
        const code = str.charCodeAt(prefixBack);
